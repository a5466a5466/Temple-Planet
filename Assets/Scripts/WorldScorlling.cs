...
public class WorldScorlling : MonoBehaviour
{
    ...
    private void Update() {
        ...
        if(currentTilePosition != playerTilePosition){
            ...
            onTileGridPlayerPosition.x = CalculatePositinOnAxis(onTileGridPlayerPosition.x, true);
            onTileGridPlayerPosition.y = CalculatePositinOnAxis(onTileGridPlayerPosition.y, false);
            UpdateTileOnScreen();
        }
    }
    ...
    private void UpdateTileOnScreen(){
        for(int pov_x = -(fieldOfVisionWidth/2); pov_x <= fieldOfVisionWidth/2; pov_x++){
            for(int pov_y = -(fieldOfVisionHeight/2); pov_y <= fieldOfVisionHeight/2; pov_y++){
                int tileToUpdate_x = CalculatePositinOnAxis(playerTilePosition.x + pov_x, true);
                int tileToUpdate_y = CalculatePositinOnAxis(playerTilePosition.y + pov_y, false);
    ...
    private int CalculatePositinOnAxis(float currentValue, bool horizontal){
        if(horizontal){
            if(currentValue >= 0){
    ...
    private void UpdateOnTileGridPlayerPosition(){
        if(currentTilePosition.x >= 0){
            ...
            onTileGridPlayerPosition.x = (terrainTileHorizontalCount - 1) + playerTilePosition.x % terrainTileHorizontalCount;
        }

        if(currentTilePosition.y >= 0){
            onTileGridPlayerPosition.y = playerTilePosition.y % terrainTileVertacalCount;
        }else{
            onTileGridPlayerPosition.y = (terrainTileVertacalCount - 1) + playerTilePosition.y % terrainTileVertacalCount;
        }
    }
    */
